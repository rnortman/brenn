...
- **DB-backed app access + admin UI.** Move per-app access control (`allowed_users` on each `agent` block) out of the config file into the database, with an admin interface to manage grants. Today access changes mean editing a config file and restarting (config is load-once-at-boot). Premature for the current 2-user deployment, but the right shape once multi-tenant or self-service delegation becomes real. Scope: new `(user, app)` grant table, runtime hot-reload (or per-request read) of access, admin routes + catalog UI + ts-rs schemas. Note the dual role of `allowed_users` today — it's also the "owner username" resolver for singleton apps (messaging/MQTT/webhook/automation derive owner from `allowed_users.first()`), so a migration must preserve or explicitly replace that. (Moved from TODO.md slug `db-app-access` during the 2026-06-03 burndown — too big for a TODO, real as a feature.)
