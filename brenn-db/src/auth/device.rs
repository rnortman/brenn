...
pub fn unenroll_device(conn: &Connection, device_id: i64, reason: &str) -> UnenrollOutcome {
    ...
    crate::push_subscriptions::delete_all_subscriptions_for_device(&tx, device_id);
...
/// overwrite performed at unenroll time.
...
fn upsert_device_user(conn: &Connection, device_id: i64, user_id: i64) -> bool {
    let now = crate::format_ts_for_db(Utc::now());
    conn.execute(
...
mod tests {
    ...
    use crate::init_db_memory;

    fn setup_user(conn: &Connection, username: &str) -> i64 {
        create_user(conn, username, "hash")
    }

    /// Write a `pwa_push_subscriptions` row directly.
    ///
    /// The subscribe-side DAO lives above this crate (it takes a validated
    /// endpoint), so the unenroll tests write the row they are about to assert
    /// the deletion of rather than reaching upward for it.
    fn insert_push_subscription(conn: &Connection, device_id: i64, user_id: i64, endpoint: &str) {
        conn.execute(
            "INSERT INTO pwa_push_subscriptions
                 (device_id, user_id, endpoint, p256dh_b64url, auth_b64url, created_at, last_used_at)
                 VALUES (?1, ?2, ?3, 'p256dh', 'auth', '2026-01-01', '2026-01-01')",
            rusqlite::params![device_id, user_id, endpoint],
        )
        .expect("insert push subscription");
    }

    fn subscription_exists(conn: &Connection, device_id: i64, user_id: i64) -> bool {
        use rusqlite::OptionalExtension as _;
        conn.query_row(
            "SELECT 1 FROM pwa_push_subscriptions WHERE device_id = ?1 AND user_id = ?2",
            rusqlite::params![device_id, user_id],
            |_| Ok(()),
        )
        .optional()
        .expect("query push subscription")
        .is_some()
    }
    ...
    fn unenroll_device_deletes_push_subscriptions() {
        ...
        insert_push_subscription(&conn, device_id, alice, "https://push.example.com/alice");
        insert_push_subscription(&conn, device_id, bob, "https://push.example.com/bob");
    ...
    /// The subscribe DAO lives one crate above, so the row is inserted
    /// directly; the cross-crate seam is exercised in
    /// `brenn-lib/src/pwa_push/db.rs`.
    #[test]
    fn unenroll_disconnects_subscription_count() {
        ...
        insert_push_subscription(&conn, device_id, user_id, "https://push.example.com/test");
        assert!(
