...
/// Returns a map from message_id to Vec<AttachmentMeta> (the wire type).
pub fn get_attachments_for_conversation(
    conn: &Connection,
    conversation_id: i64,
) -> std::collections::HashMap<i64, Vec<brenn_ws_types::AttachmentMeta>> {
    ...
    let mut map: HashMap<i64, Vec<brenn_ws_types::AttachmentMeta>> = HashMap::new();
    let rows = stmt
        .query_map([conversation_id], |row| {
            let message_id: i64 = row.get(1)?;
            let meta = brenn_ws_types::AttachmentMeta {
                upload_id: row.get(0)?,
                filename: row.get(2)?,
