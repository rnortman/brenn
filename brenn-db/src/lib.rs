//! SQLite data layer: the connection handle, the base schema, and the DAOs over
//! the tables that schema creates.
//!
//! The crate owns exactly the tables its own production code touches: the base
//! tables ([`run_base_migrations`]) and `pwa_push_subscriptions`
//! ([`push_subscriptions`], written here because `unenroll_device` deletes its
//! rows inside the unenroll transaction). [`run_migrations`] runs both, so any
//! caller that has run it can call any function in this crate.
...
///
/// The caller runs the migrations it needs and wraps the connection in a [`Db`].
...
pub fn open_connection_memory() -> Connection {
...
/// Wrap a connection in the app-wide mutex-guarded handle.
pub fn into_db(conn: Connection) -> Db {
    Arc::new(Mutex::new(conn))
}

/// Run every migration this crate owns: the base tables and the
/// push-subscription table. Idempotent.
///
/// This is the whole set a caller needs before calling anything in this crate.
pub fn run_migrations(conn: &Connection) {
    run_base_migrations(conn);
    push_subscriptions::run_push_subscription_migrations(conn);
}

/// Open an in-memory database carrying this crate's tables only.
#[cfg(test)]
pub(crate) fn init_db_memory() -> Db {
    let conn = open_connection_memory();
    run_migrations(&conn);
    into_db(conn)
}
...
pub fn run_base_migrations(conn: &Connection) {
    conn.execute_batch(
        "
        ...
        -- Serves the current-user correlated subquery (MAX(last_seen_at)
        -- WHERE device_id = ?) that the push-target query runs. Without it
        -- that query scans all device_users rows for a device, O(N) per
        -- outer row.
...
pub fn save_app_models(conn: &Connection, app_slug: &str, models: &[brenn_ws_types::ModelInfo]) {
...
pub fn load_app_models(conn: &Connection, app_slug: &str) -> Vec<brenn_ws_types::ModelInfo> {
    conn.query_row(
...
pub fn load_approval_rules(
    ...
    app_slug: &str,
    conversation_id: i64,
) -> Vec<brenn_approval_rules::ApprovalRuleRow> {
    let mut stmt = conn
        .prepare(
    ...
    let rows = stmt
        .query_map(rusqlite::params![app_slug, conversation_id], |row| {
            Ok(brenn_approval_rules::ApprovalRuleRow {
                id: row.get(0)?,
                app_slug: row.get(1)?,
...
pub fn load_all_app_models(
    conn: &Connection,
) -> std::collections::HashMap<String, Vec<brenn_ws_types::ModelInfo>> {
    let mut stmt = conn
    ...
    let rows = stmt
        .query_map([], |row| {
            ...
            let models: Vec<brenn_ws_types::ModelInfo> =
