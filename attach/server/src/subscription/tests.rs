...
use brenn_messaging_store::store::{ResumeCursor, StoreRetained};
use tokio::sync::mpsc;
use uuid::Uuid;
...
use crate::test_support::{AttachCtxBuilder, TestProfile, one_channel_messenger};
...
async fn seed(db: &Db, channel_uuid: Uuid, body: &str, ts_ns: i64) {
    ...
    use brenn_messaging_store::db::insert_message;
...
async fn parity_rig(db: &Db, class: Class) -> ParityRig {
    let (messenger, channel_uuid, policy) = match class {
        ...
        Class::Ephemeral => {
            let (messenger, uuid) =
                crate::test_support::one_ephemeral_channel_messenger(db, EPH_BARE, EPH_RING_DEPTH);
            let mut policy = AppPolicy::default();
...
async fn attach_runtime(
    ...
    let (messenger, channel_uuid) =
        crate::test_support::one_channel_messenger_at_standing(db, BARE, standing).await;
