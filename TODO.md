...
The `WIT_BINDGEN_VERSION` (`0.60.0`) and `WASM_TOOLS_VERSION` (`1.255.0`)
...
A third copy of both literals lives outside this repo, in the private CD
pipeline's toolchain-install step, and drifts the same way. It is not covered by
any gate here: a bump in this repo leaves the deploy build installing the older
generator, which still builds and still ships (the older generator's output is
what the older pins always produced), so the drift is silent rather than fatal.
Whatever single authoritative place the pins land in has to be readable from
there too, or that copy stays hand-synced.
...
## `wasm-world-equivalence-gate`

`check_bindings_drift` (xtask/src/check_wit.rs) regenerates each component's
bindings with the pinned `wit-bindgen-cli` and byte-compares against the
committed `bindings.rs`. Both sides of that comparison are produced by the pin,
so when the pin moves they move together and the gate passes by construction: it
catches a hand-edited or stale `bindings.rs`, and nothing about whether the
world the artifact actually carries still matches `brenn-wasm/wit/*.wit`. A
generator bump that renamed an import, reordered a variant, or changed a lowered
ABI would ship clean and surface at load time inside somebody else's
out-of-tree component.

The obvious implementation — decode the artifact, print its world and the source
world with `WitPrinter`, assert string equality — does not work, and anything
here has to decide what to do about three real, legitimate differences measured
against the current artifacts:

- **Unused imports are elided.** `brenn_replay.wasm` carries no
  `brenn:replay/config` import at all; the guest never calls it, so it is not in
  the encoded type section even though the world declares it. An artifact's
  world is a *subset* of the source world's imports, not an equal.
- **Docs are stripped.** The encoded type section carries no doc comments; the
  source WIT is full of them.
- **Item order differs.** Types and functions come back in the encoder's order,
  not the source file's.

So the gate needs a defined notion of "still the same contract" — plausibly:
every import the artifact declares exists in the source world with an identical
shape, every export the source world declares is present in the artifact, and
type structure is compared modulo docs and ordering. That is a design decision
about what the guest contract permits an artifact to omit, not a mechanical
addition, which is why this is an entry rather than a patch.

Done when a generator or encoder change that moves the guest-visible world fails
`make check`, and the gate's notion of equivalence is stated where it is
implemented.

Code site (`TODO(wasm-world-equivalence-gate)`): xtask/src/check_wit.rs,
`check_bindings_drift` (the gate whose coverage stops short).

---

## `wasm-prev-generator-load-coverage`

Every WASM fixture in the tree is built by the currently pinned toolchain, so
the host-side suite only ever exercises new-host/new-guest. Out-of-tree
components are a first-class extension surface and their authors lag the pin by
definition, which makes new-host/previous-generator-guest the pairing that
actually ships in the field and the one pairing with zero automated coverage. A
host-side regression that only affects the older embedded type encoding is
invisible in CI and first observed as somebody's component failing to load after
a deploy.

The wasmtime 45→47 / wit-bindgen 0.58→0.60 bump was checked by hand — a
`components/replay` artifact rebuilt with wit-bindgen-cli 0.58.0 and wasm-tools
1.249.0 loaded and passed the replay suites under wasmtime 47 — but that was a
one-time manual check, not a gate, and nothing repeats it at the next bump.

Both ways of fixing it need a call that is not the implementer's to make:
committing a golden `.wasm` built by the previous toolchain contradicts the
deliberate no-committed-binaries/hermetic posture the test fixtures are built
on (see the note at xtask/src/check_wit.rs's WAT fixtures), and building the
same component twice with two generator versions means installing and pinning a
second toolchain in CI and in the private CD pipeline.

Done when a load-time regression against a previous-generator component fails a
gate, or the risk is accepted in writing with the acceptance recorded where the
fixtures are built.

Code site (`TODO(wasm-prev-generator-load-coverage)`): brenn-wasm/tests/grants.rs,
`drift_guard_all_fixture_imports_recognized` (the test that walks every fixture,
all of them current-generation).

---

## `wasm-table-limit-e2e`

`table_element_cap_is_per_table_and_traps_on_excess` and
`instance_and_table_count_caps_reach_the_limiter`
(brenn-wasm/tests/consume_engine.rs) assert the table-element, table-count and
instance caps against wasmtime's own `ResourceLimiter` implementation, using the
same `processor_store_limits()` value every activation runs under. What they do
not do is what `memory_cap_excess_traps_deterministically` does for memory:
drive a guest that actually grows a table past the cap and observe the trap
surface as `ProcessorOutcome::Trap`. So "wasmtime's limiter enforces the value"
is pinned; "the store consults the limiter for tables and instances" is not.

Closing it needs a fixture that executes `table.grow`, and the Rust guests the
build produces cannot: `wasm32-unknown-unknown` Rust emits no table-growth
instructions at all. That means a hand-written WAT component implementing the
processor world — a fixture class the build does not have. Every component crate
today is a Rust crate with `package.metadata.component.target`, a committed
`bindings.rs`, a Makefile build rule, and the WASI-free and bindings-drift gates
keyed off that shape; a WAT-authored artifact fits none of it and the tree
commits no `.wasm` binaries. How such a fixture is built, gated and stored is
the decision this entry is waiting on.

Done when a guest that grows a table past `PROCESSOR_MAX_TABLE_ELEMENTS` yields
`ProcessorOutcome::Trap` carrying wasmtime's table-growth phrase, the same way
the memory cap is covered today.

Code site (`TODO(wasm-table-limit-e2e)`): brenn-wasm/tests/consume_engine.rs,
on both limiter tests.
