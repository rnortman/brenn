//! Host-side git plumbing for keeping a managed clone fresh.
//!
//! Distinct from [`crate::ops`], which is the LLM-facing (possibly
//! containerized) tooling surface. A sync pull always runs on the host, so
//! nothing here routes through `run_in_app_env` or picks a container.
//!
//! Upstream is hard-coded to `origin/main`.

use std::path::Path;

use brenn_lib::util::cap_oneline;
use tracing::{debug, warn};

use crate::subprocess::{GitSubprocessError, run_git};
...
/// continue with an unclassified HEAD state.
    ...
    match is_ancestor_result {
        Ok(true) => {
            if new_head == prev_head {
                // Clone was already ahead of origin/main; the merge was a no-op.
...
/// pinning the panic message wording against regressions.
///
...
pub async fn pull_clone(clone_path: &Path) -> PullOutcome {
    ...
    if new_head != remote_head {
        ...
        //
        //   Case A — concurrent local commit landed between merge and rev-parse
        //   (e.g., `brenn_git::ops::repo_commit_and_push` running outside the sync
        ...
        match classify_post_merge(&prev_head, &remote_head, &new_head, is_anc) {
            PostMergeOutcome::UpToDate => {
                // Clone was already ahead of origin/main; the ff-only merge
            ...
            PostMergeOutcome::Advanced => {
                ...
                // code path. The classifier logic that routes here is
                // covered by `classify_case_a_classic`, and the
...
/// subprocess plumbing.
pub async fn rev_parse(clone_path: &Path, rev: &str) -> Result<String, String> {
...
pub fn oneline_unavailable(prev: &str, new: &str) -> Vec<String> {
    vec![format!(
        "{}..{} <oneline unavailable>",
        short(prev),
        short(new)
...
mod tests {
    use super::*;
    use git_fixture::scratch::scratch_remote_and_clone;
    ...
    async fn collect_oneline_caps_at_ten_with_overflow_marker() {
        ...
        use brenn_lib::util::ONELINE_CAP;
    ...
    // -----------------------------------------------------------------------
    // is_ancestor helper tests
    // -----------------------------------------------------------------------

    #[tokio::test]
    async fn is_ancestor_descendant_chain() {
        use git_fixture::scratch::{head, local_commit};
        let (_remote, clone) = scratch_remote_and_clone();
    ...
    async fn is_ancestor_unrelated_tips() {
        use git_fixture::scratch::orphan_commit;
        let (_remote, clone) = scratch_remote_and_clone();
    ...
    // -----------------------------------------------------------------------
    // classify_post_merge unit tests
    // -----------------------------------------------------------------------
    ...
    fn classify_case_a_local_already_ahead() {
        ...
        // The ff-only merge was a no-op: the clone already contained origin/main.
