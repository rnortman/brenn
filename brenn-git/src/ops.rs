...
pub async fn repo_status(mount: &ResolvedMount, working_dir: &Path) -> RepoStatusResult {
    ...
    let error = if errors.is_empty() {
        ...
        warn!(
            ...
            crate::subprocess::sanitize_log_line(&joined)
        );
        Some(joined)
    };
...
pub async fn repo_commit_and_push(
    ...
    .await
    {
        warn!(
            ...
            crate::subprocess::sanitize_log_line(&format!("{e}"))
        );
    ...
    match commit_result {
        Ok(stdout) => {
            ...
            info!(
                ...
                crate::subprocess::sanitize_log_line(stdout.trim())
            );
        }
        Err(
            ref e @ crate::subprocess::GitSubprocessError::NonZero {
                ref stdout,
                ref stderr,
                ..
            },
    ...
    match push_result {
        Ok(msg) => {
            info!(
                ...
                crate::subprocess::sanitize_log_line(msg.trim())
            );
            CommitPushResult {
                slug: mount.slug.clone(),
                had_changes: true,
...
pub async fn repo_run(
    ...
    match result {
        ...
        Err(crate::subprocess::GitSubprocessError::NonZero {
            stdout,
            stderr,
            exit_code,
            ..
            ...
            let stdout = crate::subprocess::strip_trailing_newline(stdout);
            GitRunResult {
                slug: mount.slug.clone(),
...
///
/// Delegates to `crate::subprocess::run_with_bounded_output_lazy` for the
...
async fn run_git_string(
    ...
    working_dir: &Path,
    container_spawn: Option<&ContainerSpawnConfig>,
) -> Result<String, crate::subprocess::GitSubprocessError> {
    let cmd = run_in_app_env("git", args, working_dir, container_spawn, &[], &[]);
    crate::subprocess::run_with_bounded_output_lazy(cmd, || git_label(args)).await
}
...
async fn run_git_string_lossy(
    ...
    let cmd = run_in_app_env("git", args, working_dir, container_spawn, &[], &[]);
    crate::subprocess::run_with_lossy_output_lazy(cmd, || git_label(args)).await
}
