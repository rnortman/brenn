...
use brenn_db::conversation::{create_conversation, get_conversation_opt};
...
mod tests {
    ...
    use brenn_db::conversation::get_conversation_opt;
    use rusqlite::OptionalExtension;

    /// Insert a user row and return the row_id.
    async fn insert_user(db: &brenn_db::Db, username: &str) -> i64 {
    ...
    async fn insert_event_conv(db: &brenn_db::Db, app_slug: &str, user_id: i64) -> i64 {
        let conn = db.lock().await;
        let conv_id = brenn_db::conversation::create_conversation(&conn, user_id, app_slug, false);
        conn.execute(
    ...
    async fn insert_disabled_job(db: &brenn_db::Db, app_slug: &str) -> i64 {
        let conn = db.lock().await;
    ...
    async fn insert_enabled_jobs(db: &brenn_db::Db, app_slug: &str, count: u32) -> Vec<i64> {
        let conn = db.lock().await;
        let now = brenn_db::format_ts_for_db(Utc::now());
        let mut ids = Vec::new();
        for i in 0..count {
    ...
    async fn read_mapping(db: &brenn_db::Db, app_slug: &str) -> Option<i64> {
        let conn = db.lock().await;
        conn.query_row(
    ...
    async fn a_rebind_announces_the_conversation_it_creates() {
        ...
        let engine = make_engine_on_the_bus(
            ...
            Arc::new(apps),
            vec![],
            crate::test_support::FakeIngressRouter::new(),
        )
        .await;
    ...
    async fn deleted_conversation_mapping_cleaned_up() {
        ...
        let stale_conv_id = {
            let conn = db.lock().await;
            let conv_id =
                brenn_db::conversation::create_conversation(&conn, user_id, "myapp", false);
            conn.execute(
                "INSERT INTO automation_app_event_conversation (owner_app_slug, conversation_id) \
                 VALUES ('myapp', ?1)",
    ...
    async fn mixed_scenario() {
        ...
        stale_cfg.messaging = Some(brenn_lib::messaging::config::ResolvedMessagingConfig {
            send_budget: 100,
            subscriptions: vec![],
        });
        apps.insert("stale-app".to_string(), stale_cfg);
        let mut healthy_cfg = default_app_cfg("healthy-app", false);
        healthy_cfg.messaging = Some(brenn_lib::messaging::config::ResolvedMessagingConfig {
            send_budget: 100,
            subscriptions: vec![],
        });
