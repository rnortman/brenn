...
pub async fn run_startup_catchup(engine: &AutomationEngine) {
    ...
    for job in due_jobs {
        ...
        if is_persistent {
            ...
            let last_fired_str = most_recent_past.map(brenn_db::format_ts_for_db);
            let next_str = match next_future {
                Some(t) => brenn_db::format_ts_for_db(t),
                None => {
                    ...
                    conn.execute(
                        "UPDATE automation_jobs \
                         ...
                         WHERE id = ?2",
                        rusqlite::params![brenn_db::format_ts_for_db(now), job.row_id],
                    )
                    ...
                    conn.execute(
                        ...
                        rusqlite::params![job.row_id, brenn_db::format_ts_for_db(now)],
                    )
                    .expect("insert unsatisfiable-startup fire record");
                    drop(conn);

                    // Report to owner so they learn about the auto-disable.
                    let fire_time_str = brenn_db::format_ts_for_db(now);
...
mod tests {
    ...
    fn make_engine_for_catchup(db: brenn_db::Db) -> Arc<AutomationEngine> {
        make_engine_full(
            db,
    ...
    fn make_engine_for_catchup_with_ingress_router(
        ...
        let engine = make_engine_full(
            db,
            MessagingDirectory::new(),
            ingress_router.clone() as Arc<dyn crate::IngressRouter>,
            Arc::new(FakeWakeRouter),
            AlertDispatcher::noop().0,
            AutomationGlobalConfig::default(),
            true,
    ...
    fn insert_job_due_past(
        ...
        let next_fire_at_str = brenn_db::format_ts_for_db(next_fire_at);
        let last_fired_at_str = last_fired_at.map(brenn_db::format_ts_for_db);
        let created_at_str = brenn_db::format_ts_for_db(created_at);
    ...
    async fn restart_no_missed_slots_is_noop() {
        let db = init_db_memory();
        let future_fire = brenn_db::format_ts_for_db(Utc::now() + chrono::Duration::minutes(30));
        {
            let conn = db.lock().await;
            insert_test_user(&conn);
