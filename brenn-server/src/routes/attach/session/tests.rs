...
use crate::routes::attach::registry::{AttachSessionHandle, PUSH_QUEUE_FRAMES, SessionCaps};
use crate::test_support::attach::{
...
impl Attachment {
    ...
    fn start() -> Self {
        Self::start_with(
            TestProfile::new(),
            crate::test_support::surface::shape_only_messenger(),
            AppPolicy::default(),
            IDLE_HEARTBEAT_SECS,
        )
    }
...
async fn subscribing_attachment(db: &Db, heartbeat_secs: u32) -> (Attachment, Uuid) {
    let (messenger, channel_uuid) =
        crate::test_support::attach::one_channel_messenger(db, CHANNEL_BARE).await;
...
async fn live_push(attachment: &Attachment, seq: u64) -> SessionPush {
    ...
    SessionPush::Live(crate::routes::attach::registry::LiveDelivery {
...
async fn co_available_pushes_are_coalesced_into_one_batch() {
    ...
    let view = SessionPush::DeferredView(crate::routes::attach::registry::DeferredViewPush {
        channel: CHANNEL.to_string(),
        attribution: None,
        entries: Vec::new(),
    });
