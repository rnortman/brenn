...
//! crate-local beside it (`routes::attach`'s suites, over a driven duplex with a
...
async fn surface_ws_subscribe_gives_no_existence_oracle_across_surfaces() {
    ...
    let SurfaceTestHarness {
        ...
        flusher,
        ..
    } = super::test_fixtures::surface_harness_with_siblings(
        &db,
        deskbar_loop(vec![]),
        vec![otherbar()],
        vec![
...
fn otherbar() -> brenn_lib::messaging::config::ResolvedSurface {
    crate::test_support::surface::SurfaceFixture::new("otherbar", COMPONENT)
...
fn spanning_connection_config() -> brenn_lib::config::BrennConfig {
    ...
    let status_channel = ChannelConfigRaw {
        address: Some(super::description::surface_status_bare(
            &crate::test_support::surface::description_params().prefix,
            "deskbar",
        )),
        send_rate: None,
