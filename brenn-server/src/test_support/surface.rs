...
///
/// For runtime-shape fixtures — page rendering, binding maps, reserved-port
/// wiring — that carry wire bindings and so must satisfy `SurfaceRuntime::build`'s
/// boot invariant, but never project a message through the thing. A fixture that
/// actually delivers wants the channel-carrying `fixture_messenger` instead.
pub(crate) fn shape_only_messenger() -> std::sync::Arc<brenn_lib::messaging::Messenger> {
    brenn_lib::messaging::Messenger::new(
        brenn_lib::db::init_db_memory(),
        std::sync::Arc::new(brenn_lib::messaging::MessagingDirectory::with_entries(
            vec![],
        )),
        std::sync::Arc::from("test"),
        std::sync::Arc::new(indexmap::IndexMap::new()),
        std::sync::Arc::new(crate::messaging_router::WakeRouterImpl::new(
...
impl SurfaceFixture {
    ...
    pub(crate) fn build(mut self) -> ResolvedSurface {
...
pub(crate) fn derive_wire_subscriptions(surface: &mut ResolvedSurface) {
...
pub(crate) fn bind_wire_subscription_uuids(
