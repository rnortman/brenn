...
use crate::messaging::config::{Depth, MessagingGlobalConfig, ResolvedChannel};
use crate::messaging::{
...
mod tests {
    ...
    use crate::messaging::{MessagingDirectory, WakeRouter};
    ...
    async fn messenger(with_roster_channels: bool) -> Arc<Messenger> {
        ...
        for slug in [APP, OTHER] {
            let mut app = crate::messaging::test_support::test_app_config(slug, None, vec![]);
