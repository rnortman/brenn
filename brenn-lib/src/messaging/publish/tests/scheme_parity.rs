...
use crate::messaging::config::{Depth, MessagingGlobalConfig, ResolvedMessagingConfig, SendRate};
use crate::messaging::store::RingStores;
use crate::messaging::testutils::{
    ephemeral_channel_entry, local_channel_entry, test_channel_entry,
};
use crate::messaging::{MessagingDirectory, WakeRouter, canonical_address, db::upsert_channels};
...
fn ring_store(m: &Messenger, address: &str) -> Arc<crate::messaging::store::RingStore> {
    m.ring_stores()
...
fn publisher(slug: &str) -> crate::config::AppConfig {
...
async fn retained_bodies(m: &Messenger, addr: &str) -> Vec<String> {
    m.store_for_address(addr)
        .retained_tail(crate::messaging::config::Depth::Bounded(10))
        .await
        .into_iter()
        .map(|e| e.body.clone())
        .collect()
...
async fn send_budget(m: &Messenger) -> u32 {
    ...
    crate::messaging::db::read_send_budget(&conn, CONVERSATION).expect("the budget row exists")
}
