...
pub enum SecurityEventType {
    ...
    /// rate. Signals a self-echo/runaway loop or an over-active consumer.
    /// Attributed to the component via `log_component_security_event` — no
    /// `ip` (the "attacker" is an out-of-tree guest, not a bannable peer, so
    /// fail2ban's failregex never matches), no `conversation_id` (WASM
    /// consumers have none). The response channel is the phone alert (once per
    /// process per slug).
...
mod tests {
    ...
    async fn signal_dedups_alert_per_app_origin_slug_kind() {
        let (dispatcher, captured, handle) =
            crate::alerting::make_capturing_alerter_with_severity();
    ...
    async fn signal_component_body_omits_conversation_clause() {
        let (dispatcher, captured, handle) = crate::alerting::make_capturing_alerter();
        signal_publish_denial(
            &dispatcher,
