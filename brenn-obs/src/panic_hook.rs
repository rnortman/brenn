...
use crate::alerting::{AlertDispatcher, AlertSeverity, PanicMailConfig, send_panic_mail_sync};
use crate::build_alert_context;
use crate::config::{AlertBackend, ObsConfig};
...
/// `obs_config::build` returns, before `init`.
///
...
mod tests {
    ...
    use crate::alerting::SEND_PANIC_MAIL_SYNC_CALL_COUNT;
    use crate::config::{AlertBackend, AlertConfig, ObsConfig, RateLimitConfig};
    ...
    async fn at_most_one_send_after_full_hook_supersedes_pending() {
        use crate::alerting::{PanicMailConfig as Pmc, noop_alert_dispatcher};
