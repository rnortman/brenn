...
pub struct AttachSendDraw<'a> {
    ...
    /// [`brenn_lib::token_bucket`]. Boot's sizing invariant is what keeps a maximal
...
enum PublishPrincipal<'a> {
    ...
    /// [`AttachScope`](crate::AttachScope) names. Always paired with
...
impl Messenger {
    ...
    /// (`brenn_lib::token_bucket`): a draw the balance does not cover whole is
    ...
    async fn publish_gated(
        ...
        let (policy_ref, conversation_send_budget) = match principal {
            PublishPrincipal::App { slug } => {
                let app = match resolve_publish_sender(&apps, slug, grant) {
                    Some(a) => a,
                    None => return PublishResult::MissingSender,
                };
                (Arc::clone(&app.policy), Some(app.messaging_send_budget()))
            }
        ...
        let policy: &AppPolicy = &policy_ref;
    ...
    pub(crate) fn resolve_attach_feed_targets(
        &self,
        channel_address: &str,
        subscribers: &[crate::SubscriberEntry],
...
impl Messenger {
    ...
    /// mqtt, webhook, and automation error-report callers.
    ///
