...
pub(super) struct CountingRouter {
    ...
    pub(super) fed: tokio::sync::Mutex<Vec<Arc<crate::MessageEnvelope>>>,
...
impl WakeRouter for CountingRouter {
    async fn deliver(
        &self,
        key: &crate::SubscriberEntryKind,
        envelope: &Arc<crate::MessageEnvelope>,
        _retained_seq: i64,
    ) -> Result<bool, String> {
        use crate::format::format_messaging_event_single;
...
pub(super) struct ChatFixture {
    ...
    pub(super) apps: IndexMap<String, brenn_lib::config::AppConfig>,
...
pub(super) async fn build_messenger_with(
    ...
    surface_policies: std::collections::HashMap<String, brenn_lib::access::AppPolicy>,
) -> (Arc<Messenger>, Uuid, i64, i64, Arc<CountingRouter>) {
    let db = init_db_memory();
    let conn = db.lock().await;
    // Users / conversations.
