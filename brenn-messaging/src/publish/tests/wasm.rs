...
//! integration tests, including push-target resolution.
...
/// matcher. The delivery-time ACL gate denies any
...
fn wasm_delivery_policies(
    slugs: &[&str],
) -> std::collections::HashMap<String, brenn_lib::access::AppPolicy> {
    ...
    slugs
        ...
        .map(|slug| {
            (
                slug.to_string(),
                crate::test_support::brenn_delivery_policy(ChannelMatcher::Prefix(String::new())),
            )
        })
        .collect()
}
...
async fn surface_feed_targets_cover_both_depths() {
    ...
    let policy = crate::test_support::brenn_delivery_policy(ChannelMatcher::Prefix(String::new()));
    ...
    .with_subscriber_registrations(crate::testutils::surface_registrations(
...
async fn attach_feed_targets_cover_a_remote_subscriber() {
    ...
    .with_subscriber_registrations(crate::testutils::remote_registrations(
...
async fn attached_feed_targets_see_an_entry_minted_after_the_callers_snapshot() {
    ...
    let db = init_db_memory();
    let policy = crate::test_support::brenn_delivery_policy(ChannelMatcher::Prefix(String::new()));
    let entry = crate::testutils::test_channel_entry("late-subscriber", vec![]);
    let uuid = entry.uuid;
...
async fn attached_feed_targets_fall_back_to_the_snapshot_for_a_vanished_channel() {
    ...
    let db = init_db_memory();
    let policy = crate::test_support::brenn_delivery_policy(ChannelMatcher::Prefix(String::new()));
    let entry = crate::testutils::test_channel_entry(
        "vanishing",
        vec![SubscriberEntry {
            kind: SubscriberEntryKind::Remote("pod-kitchen".to_string()),
            push_depth: Depth::Bounded(8),
...
async fn build_wasm_messenger(
    ...
    .with_subscriber_registrations(crate::testutils::wasm_registrations(
    ...
    messenger
        .attach_subscriber(
            ...
            wasm_slug,
            &crate::ParticipantId::for_wasm(wasm_slug),
            push_depth,
        )
        .await;
...
async fn wasm_and_app_subscriber_are_both_owed_the_message() {
    ...
    // has-something probe.
    let wasm_owed =
        crate::testutils::owed_everywhere(&m, &ParticipantId::for_wasm(wasm_slug)).await;
    assert_eq!(
        wasm_owed
            .iter()
    ...
    let app_owed = crate::testutils::owed_everywhere(&m, &ParticipantId::for_conversation(2)).await;
    assert_eq!(
        app_owed
            .iter()
            .map(|(_, e)| e.body.as_str())
...
async fn wasm_subscriber_is_owed_the_published_message() {
    ...
    let owed = crate::testutils::owed_everywhere(&m, &ParticipantId::for_wasm(slug)).await;
    assert_eq!(
        owed.iter()
            .map(|(_, e)| e.body.as_str())
            .collect::<Vec<_>>(),
...
async fn publish_from_wasm_two_publishes_correct_fields() {
    ...
    let owed =
        crate::testutils::owed_everywhere(&m, &ParticipantId::for_wasm("wasm-output-receiver"))
            .await;
    assert_eq!(
        owed.iter()
            .map(|(_, e)| e.body.as_str())
...
async fn build_wasm_mixed_output_messenger_at_retain(
    ...
    use crate::testutils::ephemeral_channel_entry;
...
// -----------------------------------------------------------------------
// wake_min × eager_wake integration
// -----------------------------------------------------------------------
