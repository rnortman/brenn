...
fn surface_channel_entry(subscribers: Vec<SubscriberEntry>) -> ChannelEntry {
    ChannelEntry {
        ...
        resolved_channel: ResolvedChannel {
            ...
            send_rate: crate::config::SendRate {
                burst: u32::MAX,
...
async fn assemble_surface_messenger_router(
    ...
    let messenger = messenger
        ...
        .with_attach_send_budgets(budget_principals.into_iter().flat_map(|(slug, ps)| {
            crate::attach_principal_budgets(AttachScope::surface(&slug), ps)
        }));
    ...
    for slug in &wasm_subscribers {
        messenger
            .attach_subscriber(
                &channel_addr,
                slug,
                &crate::ParticipantId::for_wasm(slug),
                Depth::Unbounded,
            )
            .await;
    }
...
async fn publish_from_surface_ok_stamps_surface_sender_no_budget() {
    ...
    let owed =
        crate::testutils::owed_everywhere(&m, &ParticipantId::for_wasm("surface-out-receiver"))
            .await;
    assert_eq!(
        owed.iter()
...
async fn a_repeated_slug_panics() {
    ...
    let _ = messenger.with_attach_send_budgets(
        ...
        .into_iter()
        .chain(crate::attach_principal_budgets(
            AttachScope::surface("durabar"),
            default_principals(&["clock"]),
        )),
    );
...
async fn surface_send_budget_missing_panics() {
    ...
    let m = m.with_subscriber_registrations(crate::testutils::surface_registrations({
...
async fn build_ephemeral_surface_feed_messenger_at(
    ...
    let mut entry = crate::testutils::ephemeral_channel_entry("surface-out-ch", retain);
    entry.resolved_channel.send_rate = crate::config::SendRate {
        burst: u32::MAX,
        refill_interval_secs: 1,
        refill: u32::MAX,
    };
...
async fn a_batch_overrunning_the_ring_enacts_its_overflow_events() {
    ...
    assert_eq!(
        m.drop_counter(&addr, &crate::ParticipantId::for_wasm("ring-watcher")),
        2,
