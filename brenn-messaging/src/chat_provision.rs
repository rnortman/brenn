...
use crate::config::{Depth, ResolvedChannel, SendRate};
...
fn chat_channel_entry(
    ...
    leaf: ChatLeaf,
    conversation_id: i64,
    defaults: &crate::config::MessagingGlobalConfig,
) -> ChannelEntry {
    ...
    let (send_rate, retain_depth, wake_min, push_depth, noise) = match leaf {
        ...
        ChatLeaf::Wake => (
            ...
            WakeMin::VeryLow,
            Depth::Bounded(1),
            crate::config::NoiseLevel::Silent,
        ),
...
/// [`crate::db::primed_position`], the same function the durable
...
fn ensure_command_cursor(
    ...
    let primed = crate::db::primed_position(conn, entry.uuid, push_depth);
    crate::db::ensure_subscriber_cursor(
        conn,
        entry.uuid,
        &crate::ParticipantId::for_conversation(conversation_id),
        app_slug,
        push_depth,
        primed,
    );
...
fn ensure_ring_cursor(
    ...
    let depth = crate::store::depth_bound(subscriber.push_depth);
...
mod tests {
    ...
    impl WakeRouter for RecordingRouter {
        ...
        fn delivery_shape(&self, key: &SubscriberEntryKind) -> crate::DeliveryShape {
    ...
    async fn messenger() -> (Arc<Messenger>, brenn_db::Db) {
    ...
    async fn messenger_watching_wakes() -> (Arc<Messenger>, brenn_db::Db, Arc<RecordingRouter>) {
    ...
    async fn messenger_over(db: brenn_db::Db) -> (Arc<Messenger>, Arc<RecordingRouter>) {
        let mut apps: IndexMap<String, AppConfig> = IndexMap::new();
        let mut app = crate::test_support::test_app_config(
            APP,
            Some(crate::config::ResolvedMessagingConfig {
                send_budget: 100,
                subscriptions: vec![],
            }),
            vec!["bob".to_string()],
    ...
    async fn re_provisioning_preserves_the_record_and_the_ring() {
        ...
        for addr in [&out, &stream] {
            let result = m
                ...
                .await;
            assert!(
                matches!(result, crate::publish::PublishResult::Ok { .. }),
                "{addr}: {result:?}"
            );
        }
        let ring_before = m
    ...
    fn seed_dynamic_subscription(conn: &Connection, channel: Uuid, app_slug: &str) {
        crate::db::insert_dynamic_subscription(
            conn,
            &crate::db::DynamicSubscriptionRow {
                channel_uuid: channel,
                app_slug: app_slug.to_string(),
                push_depth: Depth::Bounded(4),
                retain_depth: Depth::Bounded(4),
                noise: crate::NoiseLevel::Silent,
                wake_min: crate::WakeMin::Normal,
                qos: None,
    ...
    async fn teardown_removes_the_channels_and_their_contents() {
        ...
        for conversation_id in [7, 8] {
            let result = m
                ...
                .await;
            assert!(matches!(result, crate::publish::PublishResult::Ok { .. }));
        }
        ...
        let result = m
            ...
            .await;
        assert!(
            matches!(result, crate::publish::PublishResult::UnknownChannel(_)),
            "{result:?}"
        );
    }
