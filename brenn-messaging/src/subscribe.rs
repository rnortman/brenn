...
use brenn_lib::mqtt::address::{parse_mqtt_address, validate_topic_filter_str};

/// Raw (pre-resolution) parameters for a runtime dynamic subscribe, as supplied
/// by the `MessageSubscribe` tool.
///
...
/// call); they are passed verbatim with no inheritance. `noise` and
...
pub enum RuntimeSubscribeError {
    ...
    /// is meaningless, so it is never auto-created. (For `mqtt:` an
    /// absent topic-filter channel is created rather than erroring.)
    UnknownChannel { address: String },
    /// `qos` supplied for a non-MQTT (`brenn:`/`webhook:`) address.
    ...
    /// before deriving the channel. Carries the parser's
    /// detail string.
    ...
    /// param-mutation is withheld: the caller must
...
/// re-subscribe no-op (a re-subscribe with identical resolved
...
pub enum SubscribeOutcome {
    ...
    /// Nothing was written or mutated; the caller must NOT
...
pub enum RuntimeUnsubscribeError {
    ...
    /// static nor dynamic) — there is nothing to remove.
...
/// activation layer needs both: for `mqtt:` it
...
pub struct UnsubscribeOutcome {
    ...
    /// a broker UNSUBSCRIBE only when this is `false`.
...
mod tests {
    use super::*;
    use crate::config::{MessagingGlobalConfig, ResolvedChannel, Sink};
    use crate::db::init_db_memory;
    use crate::db::{insert_message, load_dynamic_subscriptions, upsert_channels};
    ...
    impl WakeRouter for NoopRouter {
        async fn deliver(
            &self,
            _key: &crate::SubscriberEntryKind,
        ...
        async fn deliver_ingress(
            ...
            _event: &crate::ingress::Event,
        ) -> Result<bool, String> {
            Ok(false)
        }
        fn spawn_eager_wake(&self, _key: &crate::SubscriberEntryKind, _subscriber: &ParticipantId) {
        }
        fn delivery_shape(&self, key: &crate::SubscriberEntryKind) -> crate::DeliveryShape {
    ...
    async fn messenger_with_apps(
        ...
        let ring_stores = Arc::new(crate::store::RingStores::build(&nondurable));
    ...
    async fn a_nondurable_push_subscriber_is_served_from_the_ring() {
        ...
        m.store_for(&entry)
            .append(crate::store::NewMessage {
                ...
                impetus: None,
                publish_ts_ns: crate::db::utc_to_ns(chrono::Utc::now()),
            })
            .await;
    ...
    /// resolved params is an idempotent no-op success: the outcome
    ...
    /// static subs are config-managed and unshadowable. No durable
    /// row is written.
    #[tokio::test]
    ...
    async fn subscribe_mqtt_auto_created_channel_takes_the_ingress_family_default() {
        ...
        assert_eq!(
            ...
            crate::config::INGRESS_DEFAULT_RETAIN_DEPTH,
    ...
    async fn subscribe_mqtt_auto_created_channel_takes_the_operators_tuning() {
        use crate::config::{ChannelConfigRaw, build_system_channel_tuning};
        ...
        let entry = m
            ...
            .resolve(address)
            .expect("auto-created mqtt channel is a step-2 side effect of the subscribe");
        let expected = crate::config::resolve_system_channel(address, &tuning, &global);
    ...
    /// **created**: the channel becomes resolvable + listable,
    ...
    async fn a_dynamic_subscribe_is_owed_the_retained_tail_at_its_push_depth() {
        ...
        let conversation = {
            let conn = m.db.lock().await;
            let user = brenn_db::auth::user::create_user(&conn, "u", "$argon2id$fake");
            brenn_db::conversation::get_or_create_singleton_conversation(&conn, user, "graf").id
        };
        ...
        {
            let conn = m.db.lock().await;
            let row = crate::db::load_subscriber_cursor(&conn, uuid, &subscriber)
    ...
    async fn unsubscribe_removes_own_dynamic_sub() {
        ...
        let subscriber = ParticipantId::for_conversation(conversation);
        {
            let conn = m.db.lock().await;
            assert!(
                crate::db::load_subscriber_cursor(&conn, uuid, &subscriber).is_some(),
        ...
        // channel owes it nothing.
        {
            let conn = m.db.lock().await;
            assert!(
                crate::db::load_subscriber_cursor(&conn, uuid, &subscriber).is_none(),
    ...
    async fn unsubscribe_between_a_drains_read_and_its_advance_completes() {
        ...
        let ch = channel(&crate::canonical_address("heartbeat"), ChannelScheme::Brenn);
    ...
    // -----------------------------------------------------------------------
    // list_subscriptions (MessageSubscriptionList backing)
    // -----------------------------------------------------------------------
