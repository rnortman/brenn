...
mod tests {
    ...
    use crate::config::{Depth, MessagingGlobalConfig, NoiseLevel, ResolvedChannel, Sink};
    ...
    async fn conversation_of(m: &Messenger) -> i64 {
        let conn = m.db.lock().await;
        let user = brenn_db::auth::user::get_user_by_username(&conn, USER).expect("user exists");
        brenn_db::conversation::get_singleton_conversation_id(&conn, user.id, APP)
    ...
    async fn a_pull_only_subscriber_is_never_delivered_to() {
        ...
        let conversation = {
            let conn = m.db.lock().await;
            let user =
                brenn_db::auth::user::get_user_by_username(&conn, USER).expect("user exists");
            assert!(
                brenn_db::conversation::get_singleton_conversation_id(&conn, user.id, APP)
                    .is_none(),
                "a pull-only subscriber is not a delivery target and mints nothing"
            );
            brenn_db::conversation::get_or_create_singleton_conversation(&conn, user.id, APP).id
        };
    ...
    async fn chat_messenger(entries: Vec<ChannelEntry>) -> Arc<Messenger> {
        let chat = brenn_lib::config::LlmChatConfig::default();
        let defaults = MessagingGlobalConfig::default();
        let roster = crate::chat_roster::chat_roster_entry(&chat, APP, &defaults);
        let bare = roster
            .address
        ...
        let spec = crate::system::SystemParticipantSpec::publish_only(
            crate::chat_roster::CHAT_ROSTER_COMPONENT,
            ChannelScheme::Brenn,
    ...
    fn leaf_addresses(conversation: i64) -> Vec<String> {
        let chat = brenn_lib::config::LlmChatConfig::default();
        crate::chat_provision::PROVISIONED_LEAVES
            .into_iter()
    ...
    async fn a_dynamic_subscribe_mints_a_provisioned_and_announced_conversation() {
        ...
        m.subscribe_dynamic(
            APP,
            &address,
            crate::subscribe::DynamicSubscribeParams {
    ...
    async fn re_attaching_an_already_provisioned_conversation_disturbs_nothing() {
        ...
        let before = {
            let conn = m.db.lock().await;
            crate::db::load_subscriber_cursor(
                &conn,
                command,
                &ParticipantId::for_conversation(conversation),
            )
        ...
        assert_eq!(
            m.directory
                ...
                .count(),
            crate::chat_provision::PROVISIONED_LEAVES.len(),
