...
mod tests {
    ...
    use crate::db::{ensure_subscriber_cursor, load_subscriber_cursor, upsert_channels};
    ...
    fn channel(name: &str, subscribers: Vec<SubscriberEntry>) -> ChannelEntry {
        ChannelEntry {
            uuid: Uuid::new_v4(),
            address: crate::canonical_address(name),
            description: None,
    ...
    async fn a_position_above_a_used_channels_head_resets_to_that_head() {
        ...
        let m = messenger(vec![ch], router.clone()).await;
        for i in 0..4 {
            crate::testutils::insert_bus_message(
                &m,
                &entry,
                &format!("m{i}"),
                ChannelScheme::Brenn,
    ...
    async fn a_boot_that_attached(entry: &ChannelEntry) -> brenn_db::Db {
        let db = init_db_memory();
        {
            let conn = db.lock().await;
            brenn_db::auth::user::create_user(&conn, USER, "$argon2id$fake");
