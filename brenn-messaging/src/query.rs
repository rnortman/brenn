...
use super::db::{SELECT_ENVELOPE_BASE, row_to_envelope, utc_to_ns};
...
fn run_query(
    ...
    // Apply the retain_depth clamp: effective limit = min(q.limit, clamp).
...
impl crate::WakeRouter for NoopWakeRouter {
    async fn deliver(
        &self,
        _key: &crate::SubscriberEntryKind,
        _envelope: &std::sync::Arc<crate::MessageEnvelope>,
        _retained_seq: i64,
    ) -> Result<bool, String> {
        Ok(false)
    }
    ...
    async fn deliver_ingress(
        &self,
        _key: &crate::SubscriberEntryKind,
        _: &crate::ParticipantId,
        _event: &crate::ingress::Event,
    ) -> Result<bool, String> {
        Ok(false)
    }

    fn spawn_eager_wake(&self, _key: &crate::SubscriberEntryKind, _: &crate::ParticipantId) {}

    fn delivery_shape(&self, key: &crate::SubscriberEntryKind) -> crate::DeliveryShape {
        crate::default_delivery_shape(key)
    }

    fn alarm(&self, _channel: &str, _subscriber: &crate::ParticipantId, _count: u64) {}
}

#[cfg(test)]
pub mod tests {
    use super::*;
    use crate::canonical_address;
    use crate::config::{Depth, NoiseLevel, ResolvedChannel, Sink};
    use crate::db::{init_db_memory, ns_to_utc};
    use crate::db::{insert_message, upsert_channels};
    use crate::{ChannelEntry, ChannelScheme};
    use crate::{
        MessagingDirectory, MessagingGlobalConfig, SubscriberEntry, SubscriberEntryKind, Urgency,
        WakeMin, WakeRouter,
    };
    use brenn_lib::pwa_push::config::AppPwaPushBlock;
    ...
    // -----------------------------------------------------------------------
    // resolve_retain_depth_clamp tests via Messenger::query
    //
    ...
    async fn non_subscriber_clamped_to_standing_retain_depth() {
        use crate::config::{MessagingGlobalConfig, NoiseLevel, ResolvedChannel, Sink};
    ...
    /// is unclamped — non-subscriber callers are never clamped.
    #[tokio::test]
    async fn non_subscriber_unbounded_standing_is_unclamped() {
        ...
        assert_eq!(
            result.len(),
            3,
            "unbounded standing_retain_depth must not clamp beyond caller limit"
        );
    }
    ...
    async fn subscriber_clamped_to_own_retain_depth_not_standing() {
        ...
        async fn run_case(subscriber_retain: Depth, standing: Depth, expected: usize) {
            ...
            let mut sub_app = crate::test_support::test_app_config(
                "sub-app",
    ...
    async fn dynamic_subscribe_then_query_honors_retain_depth() {
        use crate::subscribe::DynamicSubscribeParams;
    ...
    async fn query_denied_existing_and_absent_channel_errors_are_indistinguishable() {
        ...
        denied_apps.insert(
            "app".to_string(),
            crate::test_support::test_app_config("app", None, vec![]),
        );
    ...
    async fn query_ephemeral_always_unknown_channel() {
        ...
        apps.insert(
            "denier".to_string(),
            crate::test_support::test_app_config("denier", None, vec![]),
        );
        apps.insert(
            "reader".to_string(),
    ...
    // Byte-identity tests: pin this crate's own query assembly — the ORDER BY /
    // LIMIT tail and the full window-snapshot SQL — against the literal text
    // spelled out here, guarding against whitespace/boundary drift. The base
    // SELECT fragment and its decoder are pinned in the crate that owns them
    ...
    async fn ephemeral_messenger(
        ...
        let entry = crate::testutils::ephemeral_channel_entry(name, retain_depth);
    ...
    async fn ephemeral_query_clamp_is_count_not_window_under_filter() {
        ...
        let mut entry = crate::testutils::ephemeral_channel_entry("chatter", 8);
    ...
    async fn nondurable_query_window_is_scheme_blind_for_local() {
        ...
        let entry = crate::testutils::local_channel_entry("box", 8);
        let ring_stores = Arc::new(RingStores::build(std::slice::from_ref(&entry)));
        let base = utc_to_ns(Utc::now());
        {
            let store = ring_stores.get(&entry.uuid).expect("registered");
