...
use crate::db::list_subscriptions_with_device_info_for_user;
...
impl PwaPushService {
    ...
    /// `pwa_push:<u>@<d>` per visible subscription. Subscriptions whose user
    /// is not the current-user on their device are excluded so the LLM never
    /// addresses a knowably-stale target.
    pub(crate) async fn list_targets_impl(&self, app_slug: &str) -> Vec<PushTargetEntry> {
        ...
        let rows: Vec<(
            ...
            crate::db::list_all_subscriptions_with_device_info(&conn)
        };
        ...
        use crate::targets::{canonical_device_address, canonical_user_address};
    ...
    pub(crate) async fn get_target_impl(
        ...
        use crate::db::get_subscription_with_device_last_seen_by_username_and_slug;
        use crate::targets::{canonical_device_address, canonical_user_address};
    ...
    pub(crate) fn public_key_b64url_impl(&self) -> &str {
        &self.config.vapid.public_b64url
    }

    /// Return a reference to the endpoint validation policy.
    pub(crate) fn endpoint_policy_impl(&self) -> &EndpointPolicy {
