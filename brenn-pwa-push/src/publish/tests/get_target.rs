//! `get_target` tests: device-address and
...
async fn make_db_for_get_target() -> brenn_db::Db {
...
async fn get_target_device_address_returns_entry_with_max_last_seen() {
    ...
    let addr = crate::targets::parse_pwa_push_address("pwa_push:alice@laptop").expect("parse");
...
async fn get_target_device_address_sub_wins_when_newer() {
    let db = make_db_for_get_target().await;
    let svc = make_service(db);
    let addr = crate::targets::parse_pwa_push_address("pwa_push:alice@phone").expect("parse");
    let result = svc.get_target("graf", &addr).await;
    let e = match result {
        GetTargetResult::Found(e) => e,
        other => panic!("expected Found, got {other:?}"),
...
async fn get_target_device_address_absent_returns_not_found() {
    ...
    let svc = make_service(db);
    let addr = crate::targets::parse_pwa_push_address("pwa_push:alice@laptop").unwrap();
    assert!(matches!(
...
async fn get_target_user_address_returns_max_across_devices() {
    let db = make_db_for_get_target().await;
    let svc = make_service(db);
    let addr = crate::targets::parse_pwa_push_address("pwa_push:alice").unwrap();
    let result = svc.get_target("graf", &addr).await;
    let e = match result {
        GetTargetResult::Found(e) => e,
        other => panic!("expected Found, got {other:?}"),
...
async fn get_target_user_access_denied_returns_forbidden() {
    ...
    let addr2 = crate::targets::parse_pwa_push_address("pwa_push:alice@laptop").unwrap();
    assert!(matches!(
