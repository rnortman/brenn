...
use crate::db::SubscriptionRow;
use crate::endpoint_validator::{RejectReason, validate_endpoint};
use crate::payload::PushPayload;
use brenn_lib::pwa_push::vapid::build_vapid_authorization;
use brenn_obs::security::log_and_alert_ssrf_attempt;
...
pub(crate) fn publish_wide_cap() -> Duration {
    #[cfg(test)]
    {
...
#[cfg(test)]
pub(crate) static PUBLISH_WIDE_CAP_OVERRIDE_MS: std::sync::atomic::AtomicU64 =
    std::sync::atomic::AtomicU64::new(u64::MAX);

impl PwaPushService {
    /// Attempt to deliver a single push to one subscription.
    #[allow(clippy::too_many_arguments)]
    pub(crate) async fn deliver_to_subscription(
        &self,
        sub: &SubscriptionRow,
        title: &str,
        body: &str,
        ...
        if let Err(reason) = validate_endpoint(&sub.endpoint, &self.config.endpoint_policy) {
            let endpoint_preview = crate::endpoint_preview(&sub.endpoint);
            let detail = format!(
        ...
        let ep_preview = crate::endpoint_preview(&sub.endpoint);
        match result {
            Err(_elapsed) => {
                tracing::warn!(
                    subscription_id = sub.id,
...
pub(crate) async fn collect_with_publish_cap(
