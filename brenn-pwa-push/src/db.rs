...
//!
//! The table's DDL lives in `brenn_db::push_subscriptions`, beside the
//! device-unenroll deletion that runs in the same transaction as the device
//! rows; this module holds the subscribe- and publish-side DAOs over it.
//!
...
//! time does NOT pre-create rows (orphan-row prevention).
//!
...
use crate::endpoint_validator::{RejectReason, ValidatedEndpoint, validate_endpoint};
...
/// (both are sensitive — treat like cookies). `p256dh_b64url`
...
impl std::fmt::Debug for SubscriptionRow {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let ep_preview = crate::endpoint_preview(&self.endpoint);
...
/// user on that device (the current-user check).
///
...
///
/// Applies the current-user filter (most-recently-seen user on the device).
...
/// the current-user on that device (most-recently-seen user on the device).
/// Returns `None` if no subscription exists or the user is stale on the device.
///
...
mod tests {
    ...
    /// The subscribe and delete halves of a subscription's life sit in
    /// different crates: `upsert_subscription` here, deletion via
    /// `unenroll_device` one crate below. Both are production DAOs, so this
    /// is the one place the seam is exercised — an unenrolled device must
    /// retain no subscription, whatever the write path wrote.
    #[test]
    fn unenroll_device_removes_a_subscription_written_by_the_subscribe_dao() {
        let db = setup();
        let conn = db.blocking_lock();
        upsert_subscription(&conn, 1, 1, &ep("https://ep.example.com"), "p256dh", "auth");
        upsert_subscription(
        ...
        assert!(subscription_exists(&conn, 1, 1));
        assert!(subscription_exists(&conn, 1, 2));

        brenn_db::auth::device::unenroll_device(&conn, 1, "seam test");

        assert!(
            !subscription_exists(&conn, 1, 1),
            "unenroll must delete the subscribing user's row"
        );
        assert!(
            !subscription_exists(&conn, 1, 2),
            "unenroll must delete every user's row on the device"
        );
        assert!(
            subscription_exists(&conn, 2, 1),
            "unenroll must leave another device's subscription alone"
        );
    }
    ...
    use crate::endpoint_validator::RejectReason;
    use crate::endpoint_validator::test_helpers::{empty_unenforced_policy, enforced_policy};
