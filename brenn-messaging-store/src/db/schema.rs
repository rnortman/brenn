...
/// (it is, via `crate::db::run_slice_migrations`). Creates, in FK-dependency order:
...
pub fn run_messaging_migrations(conn: &Connection) {
    ...
    if !brenn_db::column_exists(conn, "messaging_messages", "retained_seq") {
    ...
    if !brenn_db::column_exists(conn, "messaging_messages", "impetus") {
    ...
    if !brenn_db::column_exists(conn, "messaging_channels", "last_retained_seq") {
        conn.execute_batch(
    ...
    if !brenn_db::column_exists(conn, "messaging_channels", "resume_epoch") {
        conn.execute_batch(
            "ALTER TABLE messaging_channels
    ...
    if brenn_db::column_exists(conn, "messaging_wasm_consume_failures", "batch_push_ids") {
        conn.execute_batch(
...
fn assert_messaging_schema_current(conn: &Connection) {
    ...
    for (table, column) in SENTINELS {
        if !brenn_db::column_exists(conn, table, column) {
            panic!(
...
mod tests {
    ...
    fn boot_normalizes_only_the_messages_nothing_still_holds() {
        ...
        let future = brenn_db::format_ts_for_db(Utc::now() + Duration::hours(1));
    ...
    fn boot_release_sequences_in_release_order_not_rowid_order() {
        ...
        let older = brenn_db::format_ts_for_db(Utc::now() - Duration::hours(2));
        let newer = brenn_db::format_ts_for_db(Utc::now() - Duration::hours(1));
        conn.execute_batch(&format!(
    ...
    fn resume_migration_backfills_seq_epoch_and_high_water() {
        ...
        let future = brenn_db::format_ts_for_db(chrono::Utc::now() + chrono::Duration::hours(1));
        conn.execute_batch(&format!(
    ...
    async fn a_migrated_store_appends_and_resumes() {
        use crate::store::{ResumeCursor, RetentionStore};
        ...
        let db: brenn_db::Db = std::sync::Arc::new(tokio::sync::Mutex::new(conn));
        let store = crate::store::DbStore::new(
            db.clone(),
            a,
            "brenn:a",
        ...
        let epoch = store
            .replay_from(None, brenn_lib::messaging::config::Depth::Unbounded)
            .await
            .epoch;
    ...
    fn envelope_type_column_strings() {
        use crate::db::EnvelopeTypeColumn;
        assert_eq!(
