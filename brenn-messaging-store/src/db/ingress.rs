...
use crate::ingress::Event as IngressEvent;
...
/// `Immediate → eager_wake=1` / `None → eager_wake=0` via the urgency mapping.
...
pub fn mark_stale_undelivered_ingress_repo_sync(conn: &Connection, staleness_days: u64) -> usize {
    use crate::ingress::{MAX_REPO_SYNC_STALENESS_DAYS, REPO_SYNC_SOURCE_PREFIX};
    assert!(
    ...
    for (push_id, subscriber_str) in &orphans {
        ...
        let now_str2 = brenn_db::format_ts_for_db(chrono::Utc::now());
        conn.execute(
