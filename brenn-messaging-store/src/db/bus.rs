...
use crate::store::OverflowEvent;
use brenn_db::format_ts_for_db;
use brenn_lib::messaging::{ChannelScheme, Impetus, MessageEnvelope, ParticipantId, Urgency};
...
pub fn bus_gc_evict_channel(
    ...
    channel_uuid: Uuid,
    channel_address: &str,
    channel_envelope_type: brenn_lib::messaging::ChannelScheme,
    frontier: u64,
    sink: brenn_lib::messaging::config::Sink,
    archive_path: Option<&std::path::Path>,
) -> BusGcEviction {
    use brenn_lib::messaging::config::Sink;
...
pub fn load_channel_messages_after_seq(
    ...
    channel_uuid: Uuid,
    after_seq: i64,
    clamp: brenn_lib::messaging::config::Depth,
...
pub fn load_channel_retained_window_seq(
    conn: &Connection,
    channel_uuid: Uuid,
    limit: brenn_lib::messaging::config::Depth,
) -> Vec<(i64, MessageEnvelope)> {
...
fn depth_limit(depth: brenn_lib::messaging::config::Depth) -> i64 {
    match depth {
        brenn_lib::messaging::config::Depth::Unbounded => -1,
        brenn_lib::messaging::config::Depth::Bounded(n) => n as i64,
    }
}

/// The one retained-window query body, shared by every retention-order read.
///
/// Selects the 14 envelope columns `super::row_to_envelope` decodes (0-13) plus
...
fn read_retained_rows(
    ...
    let rows = stmt
        .query_map(params, |row| {
            let envelope = super::row_to_envelope(row)?;
            Ok(RetainedRow {
                id: row.get(14)?,
                seq: row.get(15)?,
                envelope,
...
fn row_to_ingress_push(row: &rusqlite::Row) -> rusqlite::Result<PendingPushRow> {
    ...
    Ok(PendingPushRow {
        push_id,
        message_id,
        event: crate::ingress::Event {
            id: push_id,
            conversation_id: crate::ingress::SYNTHETIC_EVENT_ID,
            source,
            summary,
            payload: body,
...
pub(super) fn row_to_message_envelope(
    ...
    let envelope_type = brenn_lib::messaging::ChannelScheme::parse(&envelope_type_str)
        .unwrap_or_else(|| {
