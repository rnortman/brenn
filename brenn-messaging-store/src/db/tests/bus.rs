...
use brenn_lib::messaging::canonical_address;
use brenn_lib::messaging::config::{Depth, Sink};
use brenn_lib::messaging::{ChannelEntry, ChannelScheme, Urgency};
...
fn release_due_for_channel_positions_the_message_and_clears_its_hold() {
    ...
    let now = Utc::now();
    let released = crate::db::release_due_for_channel(&conn, channel_uuid, now);
    ...
    assert!(
        crate::db::release_due_for_channel(&conn, channel_uuid, now).is_empty(),
...
fn earliest_channel_release_returns_some_when_a_message_is_parked() {
    ...
    let result = crate::db::earliest_channel_release(&conn, channel_uuid);
...
fn earliest_channel_release_is_none_for_an_unparked_channel() {
    ...
    assert!(
        crate::db::earliest_channel_release(&conn, a).is_none(),
    ...
    assert!(crate::db::earliest_channel_release(&conn, b).is_some());
}
...
fn earliest_channel_release_is_none_after_release() {
    ...
    insert_msg(&conn, channel_uuid, "sender", "body", Some(release_at));
    assert_eq!(
        crate::db::release_due_for_channel(&conn, channel_uuid, Utc::now()).len(),
        1
    );

    assert!(
        crate::db::earliest_channel_release(&conn, channel_uuid).is_none(),
...
fn bus_gc_evicts_lowest_retained_seq_first_late_release_survives() {
    ...
    let released = crate::db::release_due_for_channel(&conn, ch_uuid, now);
...
fn a_release_pass_assigns_seqs_in_release_order() {
    ...
    let now = Utc::now();
    let released_a = crate::db::release_due_for_channel(&conn, a, now);
    let released_b = crate::db::release_due_for_channel(&conn, b, now);
