...
import "./batch-table.js";
import type { BrennPfinBatchTable } from "./batch-table.js";
...
/**
 * Build a `<brenn-pfin-batch-table>` with N `.pfin-batch-row` children,
 * each having accept/reject buttons so the component wires up click handlers
 * in `connectedCallback`.
 */
function mountBatchTable(rowCount: number = 2): BrennPfinBatchTable {
  const el = document.createElement(
    "brenn-pfin-batch-table",
  ) as BrennPfinBatchTable;

  for (let i = 0; i < rowCount; i++) {
    const row = document.createElement("div");
    row.className = "pfin-batch-row";
 ...
 */
function acceptAllRows(el: BrennPfinBatchTable): void {
