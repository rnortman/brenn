...
fn surface_with_policy(policy: brenn_lib::access::AppPolicy) -> ResolvedSurface {
    brenn_surface_server::fixtures_config::SurfaceFixture::new("deskbar", "protobar")
        .policy(policy)
        .build()
}
...
) -> brenn_messaging::system::SystemParticipantSpec {
    brenn_messaging::system::SystemParticipantSpec {
        component: "relay",
        policy,
...
/// surfaces are always delivered eagerly, so the knob does nothing.
#[test]
...
fn surface_component_send_budget_defaults_when_undeclared() {
    ...
    assert_eq!(
        ...
        brenn_messaging::publish::SURFACE_SEND_REFILL,
    );
}
...
fn surface_sibling_instances_carry_their_own_send_budgets() {
    ...
    assert_eq!(
        budgets,
        vec![
            ("protobar", brenn_messaging::publish::SURFACE_SEND_BURST),
            ("sidecar", brenn_messaging::publish::SURFACE_SEND_BURST),
            ("chrome", brenn_messaging::publish::SURFACE_SEND_BURST),
            ("p2", 512),
        ],
    );
}
