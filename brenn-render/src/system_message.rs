...
use brenn_messaging::IngressEvent as Event;
...
pub fn render_messages_received(envelopes: &[MessageEnvelope]) -> Option<SystemMessageRender> {
    ...
    let raw = brenn_messaging::format::format_messaging_event_batch(envelopes)
    ...
    let heading = brenn_messaging::format::batch_heading(envelopes);
...
pub fn render_event_drain(events: &[Event]) -> Option<SystemMessageRender> {
    let raw = brenn_messaging::format_event_batch(events)?;
...
pub fn render_combined_drain(
    ...
    match (events.is_empty(), envelopes.is_empty()) {
        ...
        (false, false) => {
            ...
            let event_raw = brenn_messaging::format_event_batch(events)
                .expect("non-empty events: format_event_batch must produce text");
            let msg_raw = brenn_messaging::format::format_messaging_event_batch(envelopes)
                .expect("non-empty envelopes: format_messaging_event_batch must produce text");
            ...
            let msg_heading = brenn_messaging::format::batch_heading(envelopes);
...
fn build_event_drain_html(events: &[Event], extra_card_html: Option<&str>) -> String {
    ...
    // `render_idle_hook` for unknown envelope keys. The
    ...
    // every system card uses the same outer shell.
...
pub fn render_device_slug_reminder(
    ...
    let (browser_str, platform_str) =
        brenn_db::auth::device::classify_device_info(ua_str, platform);
...
pub fn render_debug_snapshot(
    snapshot: &brenn_ws_types::DebugViewportSnapshotData,
) -> SystemMessageRender {
    ...
    struct SafeGeometry<'a> {
        ...
        max_width_768: bool,
        visual_viewport: Option<&'a brenn_ws_types::VisualViewportData>,
        input: Option<&'a brenn_ws_types::RectData>,
        input_bar: Option<&'a brenn_ws_types::RectData>,
        app_main: Option<&'a brenn_ws_types::RectData>,
        pane_layout: Option<&'a brenn_ws_types::RectData>,
        message_list: Option<&'a brenn_ws_types::RectData>,
        attachment_strip: Option<&'a brenn_ws_types::RectData>,
        chip_bar: Option<&'a brenn_ws_types::RectData>,
        presence_bar: Option<&'a brenn_ws_types::RectData>,
        status_bar: Option<&'a brenn_ws_types::RectData>,
        body: Option<&'a brenn_ws_types::RectData>,
        document_element: Option<&'a brenn_ws_types::RectData>,
...
pub fn format_lint_errors_for_cc(errors: &[brenn_ws_types::TodoLintError]) -> String {
    if errors.is_empty() {
        return String::new();
    }
...
fn strip_messaging_preamble<'a>(raw: &'a str, heading: &str) -> &'a str {
    raw.strip_prefix(heading)
        .and_then(|s| s.strip_prefix(brenn_messaging::format::MESSAGING_HEADING_SEPARATOR))
        .unwrap_or_else(|| {
            panic!(
                "messaging formatter output must start with `{heading}{sep}`; got: {:?}",
                &raw[..raw.len().min(80)],
                sep = brenn_messaging::format::MESSAGING_HEADING_SEPARATOR,
            )
        })
}
...
mod tests {
    ...
    fn render_messages_received_batch() {
        ...
        // Each envelope produces an inner `brenn-message-recv`
    ...
    // Byte-identical fixed-fixture tests for the deterministic categories
    // (3, 4, 5, 7-no-prefix). These pin the exact CC stdin payload each one
    // produces.
    ...
    fn render_idle_hook_dirty_repos() {
        ...
        // Assert the unpushed-totals branch surfaces correctly.
        assert!(
    ...
    fn render_idle_hook_dirty_repos_singular_pluralization() {
        // With 1 repo the summary uses "1 repo," (no "s").
        let mut by_slug = serde_json::Map::new();
        by_slug.insert(
            "brenn".to_string(),
    ...
    fn render_idle_hook_unknown_key_falls_back() {
        ...
        // A WARN-level log naming each unknown key is required.
    ...
    fn render_ui_error_done_structured_envelope() {
        ...
        assert_eq!(got.category, brenn_ws_types::SystemMessageCategory::UiError);
    }
    ...
    fn format_lint_errors_for_cc_formats_errors() {
        use brenn_ws_types::TodoLintError;
        let errors = vec![
            TodoLintError {
    ...
    fn render_device_slug_reminder_content_and_wrapper() {
        ...
        assert_eq!(
            got.category,
            brenn_ws_types::SystemMessageCategory::DeviceSlugReminder
        );
    }

    // ── Rendering fidelity: ingress rows drain to the same output as the
    //    direct event path ──
    ...
    fn render_event_drain_ingress_matches_event_queue_path() {
        use brenn_messaging::IngressEvent;
        ...
        let expected_text = {
            let raw = brenn_messaging::format_event_batch(std::slice::from_ref(&ev))
    ...
    /// Ingress delivered via the dispatcher renders through
    ...
    /// (3) `-immediate` CSS class absent — that class is retired.
    #[test]
    ...
    fn minimal_snapshot_data() -> brenn_ws_types::DebugViewportSnapshotData {
        brenn_ws_types::DebugViewportSnapshotData::default()
    }
    ...
    fn render_debug_snapshot_category_and_card_html() {
        ...
        assert_eq!(
            r.category,
            brenn_ws_types::SystemMessageCategory::DebugSnapshot,
