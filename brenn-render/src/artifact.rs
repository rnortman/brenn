...
///
/// Mirrors [`validate_artifact_path`]'s resolution algorithm: cwd-first, then
/// slug-prefix lookup by name. Returns `None` for any kind of miss (file gone,
/// mount removed from config, root no longer canonicalisable). These cases are
/// indistinguishable to the caller by design — `None` means "no stable URL
/// available", never a security signal.
