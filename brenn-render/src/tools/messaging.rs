...
//!
//! All three messaging tools are auto-approved: the send budget is the
//! control, not per-call user approval.
...
impl AppTool for MessageSendTool {
    ...
    fn format_summary(
        ...
        // Status badge: the card shows the final outcome (delivered / budget
        // exhausted / etc.). The intercept
...
mod tests {
    ...
    /// Outcome plumbing: `_outcome.ok = true` + `remaining_budget`
    ...
    /// Failed outcomes render the error string in the badge.
    #[test]
