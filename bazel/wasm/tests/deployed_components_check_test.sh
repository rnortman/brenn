...
# is rejected, an entry with no trailing newline is still read (the shape of
# edit most likely to have just been appended), and a manifest that yields no
# entries at all fails rather than reporting nothing missing.
set -uo pipefail

names="$1"
check="$2"
tmp="${TEST_TMPDIR:?TEST_TMPDIR must be set}"
declared="brenn_replay.wasm brenn_processor_demo.wasm"
failures=0

expect() {
    local want="$1" name="$2" needle="${3:-}"
    local out rc
    out="$("$check" "$names" "$declared" "$tmp/$name" "fixture:$name" 2>&1)"
    rc=$?
