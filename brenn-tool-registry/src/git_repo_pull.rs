//! The `git-repo-pull` tool (async class).
//!
...
//! a WASM consumer (via the bus executor) both reach the same
...
use brenn_git::pull::{pull_clone, pull_outcome_to_json};
use brenn_git::sync::{CloneInfo, SyncTriggerSender};
...
mod tests {
    ...
    use super::*;
    use brenn_git::sync::SyncTrigger;
    use git_fixture::scratch::{scratch_remote_and_clone, scratch_remote_and_clone_behind_by_one};
    ...
    fn sender_with_rx() -> (SyncTriggerSender, mpsc::Receiver<SyncTrigger>) {
        ...
        (SyncTriggerSender::new(tx, slug_to_remote), rx)
    }
