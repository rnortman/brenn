...
/// Multi-port case 6: a trap row on channel[0] with a normal row on channel[1] in the
...
/// nothing published. Activation-scoped quarantine is deliberate.
#[tokio::test]
...
/// Multi-port case 1 (combined activation): 2 triggering channels both pending →
...
/// Multi-port case 2 (sampled port as pure context): triggering port (in0,
...
async fn a_denied_port_is_empty_and_unadvanced_beside_a_served_sibling() {
    ...
    let denied = brenn_messaging::Messenger::new(
        messenger.db().clone(),
    ...
    .with_subscriber_registrations(brenn_messaging::testutils::wasm_registrations(policies));
    ...
    let still_owed: Vec<String> = brenn_messaging::testutils::owed_everywhere(&denied, &wasm_sub)
        .await
        .into_iter()
...
/// Multi-port case 3 (sampled-only traffic, no activation): only the sampled channel
...
async fn multiport_sampled_only_no_push_rows_no_activation() {
    ...
    let push_rows_before = brenn_messaging::testutils::owed_everywhere(&messenger, &wasm_sub).await;
    assert!(
...
/// Multi-port case 4 (triggering port without rows gets context window): rows pending on
...
async fn multiport_triggering_port_without_rows_appears_as_context_window() {
    ...
    // one.
    brenn_messaging::testutils::consume_owed(&messenger, &out_entry.address, &out_sub).await;
...
/// Multi-port case 7 (all-or-nothing across ports): in0 has a normal row (would publish
...
/// Multi-port case 2 (Err arm on multi-port): 2-port activation where the guest returns
...
/// Multi-port case 8 (drop reporting under multi-port): overflow on in0, rows also
...
async fn processor_dual_multi_port_activation_per_port_publish_resolution() {
    ...
    let out1_rows = brenn_messaging::testutils::owed_everywhere(&messenger, &out1_sub).await;
    let out2_rows = brenn_messaging::testutils::owed_everywhere(&messenger, &out2_sub).await;
    assert_eq!(
        out1_rows.len(),
        2,
