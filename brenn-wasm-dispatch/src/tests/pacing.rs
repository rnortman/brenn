...
/// A sustained flood above 1/min_period is paced — after
/// the burst, grant-to-grant spacing equals exactly `min_period` (the stable
/// quantity, since a sleep may overshoot). Delay, not drop: `admit` always
...
/// Entry signals fire exactly once per episode — a `warn`
...
/// must re-open (re-warn/re-log) yet NOT re-alert. Pinning the per-process
/// dedup contract that a single-episode test cannot: `episode.is_some()` after the
