...
{
  "text": "<string>",
  "priority": "<very-low|low|normal|high, default normal>",
  "expires_at": "<RFC3339 timestamp, optional>",
  "format": "<plain|markdown, default plain>"
}
```

Unknown fields are ignored. The panel keeps one live slot per priority level and displays the highest-priority slot whose expiry has not passed (ordering low→high: very-low, low, normal, high). A bare-text body occupies the `normal` slot.
