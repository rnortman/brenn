...
//! `Welcome`, the config channel's `SubscribeResult`, the document's `Deliver` —
...
use brenn_attach_proto::{SUPPORTED_VERSIONS, ServerFrame, SubscribeOutcome};
...
/// One delivery on `channel` under a stated position and identity: a store dedups
/// by message id and a span refuses a `seq` it has already seen, so two
/// deliveries on one channel must differ in both.
...
/// One delivery at the first position, for a channel a test delivers on once.
