...
//!
//! This crate sits directly above `brenn-db`: it owns the `usage_sessions` and
//! `usage_events` tables and reads the base tables (`devices`, `users`,
//! `conversations`) those rows reference. Every composition point that wires it
//! runs [`run_usage_migrations`] alongside `brenn_db::run_migrations`.
//!
...
///
/// Run by every composition point that wires this crate, after
/// `brenn_db::run_migrations` — the foreign keys point at base tables.
/// Idempotent.
...
pub fn record_ws_connect(
    ...
    let now_dt = Utc::now();
    let now = brenn_db::format_ts_for_db(now_dt);
...
///
/// `device_id` and `user_id` are the attributed sender, which the caller
/// resolves through `resolve_sender_for_conversation`.
///
/// Returns the session_id.
...
pub fn prune_usage_before(conn: &Connection, before: DateTime<Utc>) {
    let cutoff = brenn_db::format_ts_for_db(before);
    let tx = conn
        .unchecked_transaction()
...
pub fn query_sessions(conn: &Connection, filter: &SessionsFilter) -> Vec<SessionRow> {
    let from = brenn_db::format_ts_for_db(filter.from);
    let to = brenn_db::format_ts_for_db(filter.to);
...
///
/// A turn that cannot be attributed returns `None`, and the caller drops the
/// usage event rather than misattributing it.
...
mod tests {
    ...
    /// This crate's slice: `brenn-db`'s tables plus the two tables here.
    ///
    /// The crates above compose a wider slice; these tests need exactly the
    /// tables the functions under test read and write, and this crate cannot
    /// reach an opener above it.
    fn init_db_memory() -> brenn_db::Db {
        let conn = brenn_db::open_connection_memory();
        brenn_db::run_migrations(&conn);
        run_usage_migrations(&conn);
    ...
    async fn setup_db() -> (brenn_db::Db, i64, i64, i64) {
    ...
    async fn add_bob(db: &brenn_db::Db) -> (i64, i64) {
        let conn = db.lock().await;
    ...
    async fn usage_session_closes_after_gap() {
        ...
        let past = brenn_db::format_ts_for_db(Utc::now() - Duration::hours(1));
        conn.execute(
            "INSERT INTO usage_sessions
                 (device_id, user_id, app_slug, conversation_id, started_at, last_activity_at,
                  ended_at, llm_turns, ui_interactions, total_cost_usd)
