...
mod tests {
    ...
    fn channel(client: &str, topic: &str, qos: u8) -> ResolvedMqttIngressChannel {
        let address = brenn_lib::mqtt::config::parsed_address_canonical(client, topic);
    ...
    impl MqttEventRouter for NullRouter {
        async fn deliver_inbound(
            ...
            _client_slug: &str,
            _topic: &str,
            _payload: crate::payload::InboundPayload,
            _qos: u8,
        ) {
        }
    }
    ...
    async fn fail_all_publishes_drains_both_deques() {
        ...
        let (tx3, rx3) = tokio::sync::oneshot::channel();
        {
            ...
            p.push_back(crate::state::PendingPublish { ack_tx: tx1 });
            p.push_back(crate::state::PendingPublish { ack_tx: tx2 });
        }
        {
            let mut inflight = handle.inflight_publishes.lock().await;
            inflight.insert(42, crate::state::PendingPublish { ack_tx: tx3 });
        }
        ...
        assert!(matches!(
            rx1.await.unwrap(),
            Err(brenn_lib::mqtt::error::MqttError::NotConnected { .. })
        ));
        assert!(matches!(
            rx2.await.unwrap(),
            Err(brenn_lib::mqtt::error::MqttError::NotConnected { .. })
...
-----END CERTIFICATE-----\n";
    ...
    fn make_test_broker(
        ...
        tls_version_min: brenn_lib::mqtt::config::TlsVersionMin,
    ...
    fn build_tls_transport_tls12_default_yields_tls_transport() {
        let broker = make_test_broker(None, brenn_lib::mqtt::config::TlsVersionMin::Tls12);
        let transport = build_tls_transport(&broker);
        assert!(matches!(transport, Transport::Tls(_)));
    }
